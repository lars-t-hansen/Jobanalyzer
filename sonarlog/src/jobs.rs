...
use crate::{postprocess_log, read_logfiles, LogEntry, StreamKey, System, Timestamp};
...
/// Given a list of file names of log files, read all the logs and return a map from StreamKey to a
/// sorted vector of the records for the StreamKey, along with the count of unfiltered records and
/// the earliest and latest timestamp seen across all logs before filtering.
///
...
) -> Result<(HashMap::<StreamKey, Vec<Box<LogEntry>>>, usize, Timestamp, Timestamp)>
where
    F: Fn(&LogEntry) -> bool,
{
    let (entries, earliest, latest, num_records) = read_logfiles(logfiles)?;
    let streams = postprocess_log(entries, filter, configs);
    Ok((streams, num_records, earliest, latest))
}
...
fn test_compute_jobs3() {
    ...
    assert!(jobs.len() == 1);
    let job = jobs
        .get(&("ml8.hpc.uio.no".to_string(), 2447150, "python".to_string()))
        .unwrap();
